...
cmd_usage="Usage: pack_hellarmate.sh COMMAND
...
cd $ROOT_PATH/packages/hellarmate || exit 1
yarn pack --install-if-needed
tar zxvf package.tgz -C .
cd $ROOT_PATH/packages/hellarmate/package || exit 1
...
cp -R package/dist "$ROOT_PATH/packages/hellarmate"

# fix for deb package build
sudo chown -R $USER "$ROOT_PATH/packages/hellarmate/package" || true
sudo chgrp -R $USER "$ROOT_PATH/packages/hellarmate/package" || true
