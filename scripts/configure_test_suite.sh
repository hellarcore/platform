...
HPNS_OWNER_PRIVATE_KEY=$(grep -m 1 "HPNS Private Key:" ${SETUP_FILE_PATH} | awk '{$1="";printf $5}')
FEATURE_FLAGS_OWNER_PRIVATE_KEY=$(grep -m 1 "Feature Flags Private Key:" ${SETUP_FILE_PATH} | awk '{$1="";printf $6}')
HELLARPAY_OWNER_PRIVATE_KEY=$(grep -m 1 "Hellarpay Private Key:" ${SETUP_FILE_PATH} | awk '{$1="";printf $5}')
...
echo "Mint 50 Hellar to the first faucet"

MINT_FILE_PATH=${PATH_TO_PROJECT_ROOT}/logs/mint.log

yarn hellarmate wallet mint --verbose --config=local_seed 50 | tee "${MINT_FILE_PATH}"
...
echo "Mint 50 Hellar to the second faucet"
...
echo "HAPI_SEED=127.0.0.1:2443:self-signed
