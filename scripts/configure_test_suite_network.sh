...
# use path from the argument ($2), otherwise default to the ../hellar-network-configs
HELLAR_NETWORK_CONFIGS_PATH="${2:-"$(realpath "$PATH_TO_ROOT/hellar-network-configs")"}"
...
if [ ! -f "$HELLAR_NETWORK_CONFIGS_PATH/$NETWORK_STRING.yml" ]; then
    echo "Hellar network config '$NETWORK_STRING' was not found in the folder ($HELLAR_NETWORK_CONFIGS_PATH)"
    echo "Either place it in $HELLAR_NETWORK_CONFIGS_PATH or specify path via second script argument"
    exit 1
fi
...
PATH_TO_CONFIGS=$(realpath $HELLAR_NETWORK_CONFIGS_PATH)

INVENTORY=${PATH_TO_CONFIGS}/${NETWORK_STRING}.inventory
CONFIG=${PATH_TO_CONFIGS}/${NETWORK_STRING}.yml

HAPI_SEED=$(awk -F '[= ]' '/^hp-masternode/ {print $5}' "$INVENTORY" | awk NF | shuf -n1)
HAPI_PORT=1443

echo "Running against node ${HAPI_SEED}"

FAUCET_ADDRESS=$(yq .faucet_address "$CONFIG")
FAUCET_PRIVATE_KEY=$(yq .faucet_privkey "$CONFIG")
HPNS_OWNER_PRIVATE_KEY=$(yq .hpns_hd_private_key "$CONFIG")
HELLARPAY_OWNER_PRIVATE_KEY=$(yq .hellarpay_hd_private_key "$CONFIG")
FEATURE_FLAGS_OWNER_PRIVATE_KEY=$(yq .feature_flags_hd_private_key "$CONFIG")

MASTERNODE_NAME=$(grep "$HAPI_SEED" "$INVENTORY" | awk '{print $1;}')

MASTERNODE_OWNER_PRO_REG_TX_HASH=$(grep "$HAPI_SEED" "$INVENTORY" | awk -F "=" '{print $6;}')
...
if [[ "$NETWORK_STRING" == "devnet"* ]]; then
  NETWORK=devnet
  INSIGHT_URL="http://insight.${NETWORK_STRING#devnet-}.networks.hellar.io:3001/insight-api/sync"
  CERT_FLAG=":self-signed"
  ST_EXECUTION_INTERVAL=5000
else
  NETWORK=testnet
  INSIGHT_URL="https://testnet-insight.hellarpro.org/insight-api/sync"
  CERT_FLAG=""
...
if [ -z "$FAUCET_ADDRESS" ] || \
    ...
    [ -z "$HPNS_OWNER_PRIVATE_KEY" ] || \
    [ -z "$FEATURE_FLAGS_OWNER_PRIVATE_KEY" ] || \
    [ -z "$HELLARPAY_OWNER_PRIVATE_KEY" ] || \
...
echo "HAPI_SEED=${HAPI_SEED}:${HAPI_PORT}${CERT_FLAG}
FAUCET_1_ADDRESS=${FAUCET_ADDRESS}
FAUCET_1_PRIVATE_KEY=${FAUCET_PRIVATE_KEY}
FAUCET_2_ADDRESS=${FAUCET_ADDRESS}
FAUCET_2_PRIVATE_KEY=${FAUCET_PRIVATE_KEY}
HPNS_OWNER_PRIVATE_KEY=${HPNS_OWNER_PRIVATE_KEY}
FEATURE_FLAGS_OWNER_PRIVATE_KEY=${FEATURE_FLAGS_OWNER_PRIVATE_KEY}
HELLARPAY_OWNER_PRIVATE_KEY=${HELLARPAY_OWNER_PRIVATE_KEY}
...
ST_EXECUTION_INTERVAL=${ST_EXECUTION_INTERVAL}
LOG_LEVEL=info
BROWSER_TEST_BATCH_INDEX=0
BROWSER_TEST_BATCH_TOTAL=0" > "${TEST_SUITE_PATH}/.env"

echo "Configured .env in $TEST_SUITE_PATH for $NETWORK_STRING from $HELLAR_NETWORK_CONFIGS_PATH"
