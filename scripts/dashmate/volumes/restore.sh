...
volume_dump_dir="$PWD/hellarmate_volumes_dump"
...
volume_args=$(cat $metadata_file | jq -r '.[].Name' | xargs -I {} echo -n "-v {}:/hellarmate_volumes/{} ")

docker run --rm \
  $volume_args \
  -v $volume_dump_dir:/backup busybox tar xf /backup/data.tar.gz -C /hellarmate_volumes
