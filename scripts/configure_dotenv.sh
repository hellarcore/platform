...
CONFIG=local
HAPI_PATH="${PACKAGES_PATH}"/hapi
DRIVE_PATH="${PACKAGES_PATH}"/rs-drive-abci
SDK_PATH="${PACKAGES_PATH}"/js-hellar-sdk
RS_SDK_PATH="${PACKAGES_PATH}"/rs-sdk
WALLET_LIB_PATH="${PACKAGES_PATH}"/wallet-lib

# Get configuration option from hellarmate config for 1st node
# Usage:
#   get_config <option>
#
# Example:
...
function get_config() {
    # We use `jq` because it's much faster than `yarn hellarmate config get`
    jq ".configs.${CONFIG}_1.$1" <"${HOME}"/.hellarmate/config.json
}

touch "${LOGS_PATH}"/mint.log

# HAPI:
cp "${HAPI_PATH}"/.env.example "${HAPI_PATH}"/.env
...
# TODO This will be removed from hellarmate. Please use hardcoded ID from contract crates
HPNS_CONTRACT_ID=$(get_config platform.hpns.contract.id)
...
HPNS_CONTRACT_ID=${HPNS_CONTRACT_ID}
...
PLATFORM_RPC_PORT=$(get_config platform.hapi.envoy.http.port)
