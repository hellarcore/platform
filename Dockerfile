...
# - drive-abci, hellarmate-helper, test-suite, hapi - final images
#
...
ENV DB_PATH=/var/lib/hellar/rs-drive-abci/db

RUN mkdir -p /var/log/hellar \
    /var/lib/hellar/rs-drive-abci/db

COPY --from=build-drive-abci /artifacts/drive-abci /usr/bin/drive-abci
COPY --from=build-drive-abci /platform/packages/rs-drive-abci/.env.example /var/lib/hellar/rs-drive-abci/.env

# Create a volume
VOLUME /var/lib/hellar/rs-drive-abci/db
VOLUME /var/log/hellar
...
RUN addgroup -g $USER_GID $USERNAME && \
    adduser -D -u $USER_UID -G $USERNAME -h /var/lib/hellar/rs-drive-abci $USERNAME && \
    chown -R $USER_UID:$USER_GID /var/lib/hellar/rs-drive-abci /var/log/hellar

USER $USERNAME

ENV RUST_BACKTRACE=1
WORKDIR /var/lib/hellar/rs-drive-abci
...
#
# STAGE: HELLARMATE HELPER BUILD
#
FROM build-js AS build-hellarmate-helper

# Install Test Suite specific dependencies using previous
# node_modules directory to reuse built binaries
RUN yarn workspaces focus --production hellarmate

#
#  STAGE: FINAL HELLARMATE HELPER IMAGE
#
FROM node:20-alpine${ALPINE_VERSION} AS hellarmate-helper

RUN apk add --no-cache docker-cli docker-cli-compose curl

LABEL maintainer="Hellar Developers <dev@hellar.io>"
LABEL description="Hellarmate Helper Node.JS"

WORKDIR /platform

COPY --from=build-hellarmate-helper /platform/.yarn /platform/.yarn
COPY --from=build-hellarmate-helper /platform/package.json /platform/yarn.lock /platform/.yarnrc.yml /platform/.pnp* /platform/

# Copy only necessary packages from monorepo
COPY --from=build-hellarmate-helper /platform/packages/hellarmate packages/hellarmate
COPY --from=build-hellarmate-helper /platform/packages/hellarpay-contract packages/hellarpay-contract
COPY --from=build-hellarmate-helper /platform/packages/wallet-lib packages/wallet-lib
COPY --from=build-hellarmate-helper /platform/packages/js-hellar-sdk packages/js-hellar-sdk
COPY --from=build-hellarmate-helper /platform/packages/js-hapi-client packages/js-hapi-client
COPY --from=build-hellarmate-helper /platform/packages/js-grpc-common packages/js-grpc-common
COPY --from=build-hellarmate-helper /platform/packages/hapi-grpc packages/hapi-grpc
COPY --from=build-hellarmate-helper /platform/packages/hellar-spv packages/hellar-spv
COPY --from=build-hellarmate-helper /platform/packages/withdrawals-contract packages/withdrawals-contract
COPY --from=build-hellarmate-helper /platform/packages/masternode-reward-shares-contract packages/masternode-reward-shares-contract
COPY --from=build-hellarmate-helper /platform/packages/feature-flags-contract packages/feature-flags-contract
COPY --from=build-hellarmate-helper /platform/packages/hpns-contract packages/hpns-contract
COPY --from=build-hellarmate-helper /platform/packages/data-contracts packages/data-contracts
COPY --from=build-hellarmate-helper /platform/packages/wasm-hpp packages/wasm-hpp

ENV HELLARMATE_HOME_DIR=/home/hellarmate/.hellarmate
ENV HELLARMATE_HELPER=1

ENTRYPOINT ["/platform/packages/hellarmate/docker/entrypoint.sh"]
...
# Install Test Suite specific dependencies using previous
# node_modules directory to reuse built binaries
RUN yarn workspaces focus --production @hellarpro/platform-test-suite
...
LABEL maintainer="Hellar Developers <dev@hellar.io>"
LABEL description="Hellar Platform test suite"
...
COPY --from=build-test-suite /platform/packages/hellarpay-contract packages/hellarpay-contract
COPY --from=build-test-suite /platform/packages/wallet-lib packages/wallet-lib
COPY --from=build-test-suite /platform/packages/js-hellar-sdk packages/js-hellar-sdk
COPY --from=build-test-suite /platform/packages/js-hapi-client packages/js-hapi-client
COPY --from=build-test-suite /platform/packages/js-grpc-common packages/js-grpc-common
COPY --from=build-test-suite /platform/packages/hapi-grpc packages/hapi-grpc
COPY --from=build-test-suite /platform/packages/hellar-spv packages/hellar-spv
...
COPY --from=build-test-suite /platform/packages/hpns-contract packages/hpns-contract
...
COPY --from=build-test-suite /platform/packages/rs-hpp packages/rs-hpp
COPY --from=build-test-suite /platform/packages/wasm-hpp packages/wasm-hpp
...
# STAGE: HAPI BUILD
#
FROM build-js AS build-hapi

# Install Test Suite specific dependencies using previous
# node_modules directory to reuse built binaries
RUN yarn workspaces focus --production @hellarpro/hapi

#
# STAGE: FINAL HAPI IMAGE
#
FROM node:20-alpine${ALPINE_VERSION} AS hapi

LABEL maintainer="Hellar Developers <dev@hellar.io>"
LABEL description="HAPI Node.JS"
...
WORKDIR /platform/packages/hapi

COPY --from=build-hapi /platform/.yarn /platform/.yarn
COPY --from=build-hapi /platform/package.json /platform/yarn.lock /platform/.yarnrc.yml /platform/.pnp* /platform/
# List of required dependencies. Based on:
# yarn run ultra --info --filter '@hellarpro/hapi' |  sed -E 's/.*@hellarpro\/(.*)/COPY --from=build-hapi \/platform\/packages\/\1 \/platform\/packages\/\1/'
COPY --from=build-hapi /platform/packages/hapi /platform/packages/hapi
COPY --from=build-hapi /platform/packages/hapi-grpc /platform/packages/hapi-grpc
COPY --from=build-hapi /platform/packages/js-grpc-common /platform/packages/js-grpc-common
COPY --from=build-hapi /platform/packages/wasm-hpp /platform/packages/wasm-hpp
COPY --from=build-hapi /platform/packages/js-hapi-client /platform/packages/js-hapi-client

RUN cp /platform/packages/hapi/.env.example /platform/packages/hapi/.env
