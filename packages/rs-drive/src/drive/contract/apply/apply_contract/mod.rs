...
impl Drive {
    ...
    /// # Returns
    ///
    /// * `Result<Vec<LowLevelDriveOperation>, Error>` - If successful, returns a `Vec<LowLevelDriveOperation>` containing the operations to be applied.
    ///   If an error occurs during the operation gathering, returns an `Error`.
    ///
    /// # Errors
    ///
    /// This function returns an error if the operation gathering fails, or if
    /// the provided drive version does not match any known versions.
