const { MerkleBlock, Transaction } = require('@hellarpro/hellarcore-lib');
...
describe('SPV-HELLAR (forks & re-orgs) deserialized headers', () => {
  before(async () => {
...
describe('SPV-HELLAR (forks & re-orgs) serialized raw headers for mainnet', () => {
  before(async () => {
...
describe('SPV-HELLAR (addHeaders) add many headers for testnet', () => {
  before(async () => {
...
describe('SPV-HELLAR (addHeaders) add testnet headers out of order', () => {
  before(() => {
    chain = new Blockchain('testnet', 10000);
    chain.initialize(utils.normalizeHeader(testnet[0]), 1);
  });
...
describe('SPV-HELLAR (addHeaders) add many headers for mainnet', () => {
  before(() => {
    chain = new Blockchain('mainnet', 10000);
    chain.initialize(utils.normalizeHeader(mainnet[0]), 1);
  });
...
describe('SPV-HELLAR (addHeaders) add mainnet headers out of order', () => {
  before(() => {
    chain = new Blockchain('mainnet', 10000);
    chain.initialize(utils.normalizeHeader(mainnet[0]), 1);
  });
...
// Hellar testnet headers which requires significant CPU power to create forked chains from
