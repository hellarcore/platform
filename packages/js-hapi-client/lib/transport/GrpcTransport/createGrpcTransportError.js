...
const { parseMetadata } = require('@hellarpro/hapi-grpc');

const {
  deserializeConsensusError,
} = require('@hellarpro/wasm-hpp');
const { default: loadWasmDpp } = require('@hellarpro/wasm-hpp');
 ...
 */
async function createGrpcTransportError(grpcError, hapiAddress) {
  await loadWasmDpp();
  ...
  if (code >= 1000 && code < 5000) {
    ...
    return new InvalidRequestDPPError(consensusError, data, hapiAddress);
  }
