...
const validateNumber = (value, name, min = NaN, max = NaN) => {
  if (typeof value !== 'number') {
    throw new HAPIClientError(`'${name}' is not a number`);
  }

  if (!Number.isNaN(min) && value < min) {
    throw new HAPIClientError(`'${name}' can not be less than ${min}`);
  }

  if (!Number.isNaN(max) && value > max) {
    throw new HAPIClientError(`'${name}' can not be more than ${max}`);
  }
};
...
function createBlockHeadersProviderFromOptions(options, coreMethods) {
  ...
  if (options.blockHeadersProvider) {
    if (options.blockHeadersProviderOptions) {
      throw new HAPIClientError("Can't use 'blockHeadersProviderOptions' with 'blockHeadersProvider' option");
    }
