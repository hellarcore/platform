...
function subscribeToBlockHeadersWithChainLocksFactory(grpcTransport) {
   ...
   * @param {HAPIClientOptions & subscribeToBlockHeadersWithChainLocksOptions} [options]
   * @returns {
  ...
  async function subscribeToBlockHeadersWithChainLocks(options = { }) {
    ...
    if (options.fromBlockHeight === 0) {
      throw new HAPIClientError('Invalid argument: minimum value for `fromBlockHeight` is 1');
    }
