...
describe('createJsonTransportError', () => {
  ...
  beforeEach(() => {
    hapiAddress = new HAPIAddress('127.0.0.1');
  });
  ...
  it('should return ServerError', () => {
    ...
    const jsonTransportError = createJsonTransportError(error, hapiAddress);

    expect(jsonTransportError).to.be.an.instanceOf(ServerError);
    expect(jsonTransportError.getHAPIAddress()).to.deep.equal(hapiAddress);
