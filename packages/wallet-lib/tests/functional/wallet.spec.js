...
const seeds = process.env.HAPI_SEED
  .split(',');
...
describe('Wallet-lib - functional', function suite() {
  ...
  describe('Account', () => {
    ...
    it('populate balance with hellar', async () => {
    ...
    it('should be able to restore wallet to the same state with a mnemonic', async () => {
      ...
      if (Object.keys(transactions).length === 0) {
        // Due to the limitations of HAPI, we need to wait for a block to be mined if we connected in the
