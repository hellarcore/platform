...
const { hellarToDuffs, coinSelection, is } = require('../../../utils');
 ...
 * @param opts.amount - Amount in hellar that you want to send
 ...
 * @param opts.change - String - A valid Hellar address - optional
...
function createTransaction(opts = {}) {
  ...
  if (_.has(opts, 'recipients')) {
    ...
    if (!opts || (!opts.amount && !opts.satoshis)) {
      throw new Error('An amount in hellar or in satoshis is expected to create a transaction');
    }
    const satoshis = (opts.amount && !opts.satoshis) ? hellarToDuffs(opts.amount) : opts.satoshis;
