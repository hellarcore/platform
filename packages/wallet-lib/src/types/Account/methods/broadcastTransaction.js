...
function impactAffectedInputs({ transaction }) {
  ...
  if (changeOutput) {
    ...
    if (!address.utxos[utxoKey]) {
      address.utxos[utxoKey] = new hellarcore.Transaction.UnspentOutput(
        {
          txId: txid,
          vout: changeIndex,
...
async function _broadcastTransaction(transaction, options = {}) {
  ...
  if (is.string(transaction)) {
    ...
    return _broadcastTransaction.call(this, new hellarcore.Transaction(rawtx));
  }

  if (!is.hellarcoreTransaction(transaction)) {
    throw new InvalidhellarcoreTransaction(transaction);
  }
...
async function broadcastTransaction(transaction, options = {
  ...
  const mempoolPropagationPromise = new Promise((resolve) => {
    const listener = ({ payload }) => {
      ...
      // https://hellarcore.readme.io/v18.0.0/docs/core-guide-transactions-transaction-malleability
