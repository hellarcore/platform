...
async function executor(forcedAddressList = null) {
  ...
  logger.silly(`HAPIClient.subscribeToAddrTx.executor[${addressList}]`);
  const fetchedUtxos = {};
...
function startExecutor() {
  const self = this;
  logger.silly('HAPIClientTransport.subscribeToAddressesTransactions.startExecutor');
  this.state.executors.addresses = setInterval(() => {
    try {
      executor.call(self);
    } catch (e) {
      logger.error('HAPIClientTransport.subscribeToAddressesTransactions.executor failed', e);
      throw e;
    }
  }, fastFetchThreshold);
}
...
module.exports = async function subscribeToAddressesTransactions(addressList) {
  logger.silly(`HAPIClient.subscribeToAddressesTransactions[${addressList}]`);
  ...
  if (!executors.addresses) {
    try {
      ...
      logger.error('HAPIClientTransport.subscribeToAddressesTransactions.startingExecutor failed', e);
      throw e;
    }
  }
};
