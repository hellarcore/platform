...
const { BlockHeader, MerkleBlock, configure } = require('@hellarpro/hellarcore-lib');
const { genesis } = require('@hellarpro/hellar-spv');
...
const initX11 = async () => {
  const x11 = await X11();
  // Configure hellarcore lib to operate with wasm x11
  configure({
    x11hash: x11,
  });
};
