...
Additionally, you can also require the utility function `const coinSelection = require('@hellarpro/wallet-lib/src/utils/coinSelection.js')` for your own usage.  
...
To implements your own algorithm, you might want to take example on the [already existing one](https://github.com/hellarpay/platform/tree/master/packages/wallet-lib/src/utils/coinSelections/strategies).  
You will need your algorithm to handle multiples parameter : 

- `utxosList` - An array consisting of multiple [unspent output](https://github.com/hellarpay/hellarcore-lib/blob/master/docs/core-concepts/unspentoutput.md).
- `outputsList` - An array consisting of multiple [Output](https://github.com/hellarpay/hellarcore-lib/blob/master/docs/core-concepts/transaction.md#handling-outputs).
