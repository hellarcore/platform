...
mod tests {
    ...
    use hpp::prelude::{Identifier, Identity};
    ...
    fn run_chain_insert_one_new_identity_and_a_contract_with_bad_update() {
        ...
        let mut contract_update_1 = json_document_to_created_contract(
            "tests/supporting_files/contract/hellarpay/hellarpay-contract-all-mutable-bad-update-skipped-position.json",
            false,
            platform_version,
        )
            .expect("expected to get contract from a json document");
    ...
    fn run_chain_block_two_state_transitions_conflicting_unique_index() {
        ...
        // We use the hpns contract and we insert two documents both with the same "name"
        // This is a common scenario we should see quite often
        let config = PlatformConfig {
            quorum_size: 100,
            execution: ExecutionConfig {
                //we disable document triggers because we are using hpns and hpns needs a preorder
        ...
        let document_op_1 = DocumentOp {
            contract: platform.drive.system_contracts.hpns_contract.clone(),
