    ...
    fn default() -> Self {
        Self {
            ...
            keys: Keys::new_random_keys_with_seed(18012014, PlatformVersion::first())
                .expect("random keys for first version can not error"), //Hellar genesis day
...
pub struct Keys {
    ...
    pub(crate) hpns_master_public_key: Vec<u8>,
    /// hex-encoded
    #[serde_as(as = "serde_with::hex::Hex")]
    pub(crate) hpns_second_public_key: Vec<u8>,

    // hellarpay contract
    /// hex-encoded
    #[serde_as(as = "serde_with::hex::Hex")]
    pub(crate) hellarpay_master_public_key: Vec<u8>,
    /// hex-encoded
    #[serde_as(as = "serde_with::hex::Hex")]
    pub(crate) hellarpay_second_public_key: Vec<u8>,
...
impl Keys {
    ...
    pub fn new_random_keys_with_seed(
        ...
        Ok(Keys {
            hpns_master_public_key: ECDSA_SECP256K1
                .random_public_key_data(&mut rng, platform_version)?,
            hpns_second_public_key: ECDSA_SECP256K1
                .random_public_key_data(&mut rng, platform_version)?,
            hellarpay_master_public_key: ECDSA_SECP256K1
                .random_public_key_data(&mut rng, platform_version)?,
            hellarpay_second_public_key: ECDSA_SECP256K1
                .random_public_key_data(&mut rng, platform_version)?,
