use hellarcore_rpc::hellarcore::consensus::encode::Error as hellarcoreConsensusEncodeError;
...
pub enum ExecutionError {
    ...
    /// An error received from hellarcore during consensus encoding.
    #[error("hellar core consensus encode error: {0}")]
    hellarcoreConsensusEncodeError(#[from] hellarcoreConsensusEncodeError),

    /// hellarcore responded with a bad response error.
    #[error("hellar core bad response error: {0}")]
    hellarcoreBadResponseError(String),
    ...
    #[error("hellar core response bls error: {0}")]
    BlsErrorFromhellarcoreResponse(BlsError),
