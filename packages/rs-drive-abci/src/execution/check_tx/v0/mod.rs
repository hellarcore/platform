...
use hpp::block::extended_block_info::v0::ExtendedBlockInfoV0Getters;
use hpp::consensus::basic::decode::SerializedObjectParsingError;
use hpp::consensus::basic::BasicError;
use hpp::consensus::ConsensusError;
use hpp::fee::epoch::GENESIS_EPOCH_INDEX;
use hpp::fee::fee_result::FeeResult;
use hpp::serialization::PlatformDeserializable;
use hpp::state_transition::StateTransition;
#[cfg(test)]
...
mod tests {
    ...
    use hpp::block::block_info::BlockInfo;
    use hpp::consensus::basic::BasicError;
    use hpp::consensus::signature::SignatureError;
    use simple_signer::signer::SimpleSigner;

    use hpp::consensus::ConsensusError;
    use hpp::hellarcore::secp256k1::Secp256k1;
    use hpp::hellarcore::{key::KeyPair, signer, Network, PrivateKey};

    use hpp::data_contract::accessors::v0::DataContractV0Getters;
    use hpp::data_contract::document_type::random_document::{
        CreateRandomDocument, DocumentFieldFillSize, DocumentFieldFillType,
    };
    use hpp::data_contracts::hpns_contract;
    use hpp::document::document_methods::DocumentMethodsV0;
    use hpp::document::DocumentV0Setters;
    use hpp::identity::accessors::{IdentityGettersV0, IdentitySettersV0};

    use hpp::identity::KeyType::ECDSA_SECP256K1;
    use hpp::identity::{Identity, IdentityV0, KeyType, Purpose, SecurityLevel};
    use hpp::prelude::{Identifier, IdentityPublicKey};
    use hpp::serialization::{PlatformSerializable, Signable};

    use hpp::state_transition::documents_batch_transition::methods::v0::DocumentsBatchTransitionMethodsV0;
    use hpp::state_transition::documents_batch_transition::DocumentsBatchTransition;
    use hpp::state_transition::identity_create_transition::methods::IdentityCreateTransitionMethodsV0;
    use hpp::state_transition::identity_create_transition::IdentityCreateTransition;
    use hpp::state_transition::identity_topup_transition::methods::IdentityTopUpTransitionMethodsV0;
    use hpp::state_transition::identity_topup_transition::IdentityTopUpTransition;
    use hpp::state_transition::identity_update_transition::v0::IdentityUpdateTransitionV0;
    use hpp::state_transition::identity_update_transition::IdentityUpdateTransition;
    use hpp::state_transition::public_key_in_creation::v0::IdentityPublicKeyInCreationV0;
    use hpp::state_transition::public_key_in_creation::IdentityPublicKeyInCreation;
    use hpp::state_transition::{StateTransition, StateTransitionLike};
    use hpp::tests::fixtures::{get_hellarpay_contract_fixture, instant_asset_lock_proof_fixture};
    use hpp::version::PlatformVersion;
    use hpp::NativeBlsModule;

    use hpp::identity::contract_bounds::ContractBounds::SingleContractDocumentType;
    use hpp::platform_value::Bytes32;
    use hpp::system_data_contracts::hellarpay_contract;
    use hpp::system_data_contracts::SystemDataContract::Hellarpay;
    ...
    fn data_contract_create_check_tx() {
        ...
        let hellarpay = get_hellarpay_contract_fixture(Some(identity.id()), protocol_version);
        let mut create_contract_state_transition: StateTransition = hellarpay
    ...
    fn document_update_check_tx() {
        ...
        let hellarpay =
            get_hellarpay_contract_fixture(Some(identity.id()), platform_version.protocol_version);
        let hellarpay_contract = hellarpay.data_contract().clone();
        let mut create_contract_state_transition: StateTransition = hellarpay
            .try_into_platform_versioned(platform_version)
            .expect("expected a state transition");
        create_contract_state_transition
        ...
        let profile = hellarpay_contract
    ...
    fn identity_update_with_non_master_key_check_tx() {
        ...
        config.abci.keys.hpns_master_public_key = master_public_key.serialize().to_vec();
        ...
        config.abci.keys.hpns_second_public_key = high_public_key.serialize().to_vec();
        ...
        let update_transition: IdentityUpdateTransition = IdentityUpdateTransitionV0 {
            identity_id: hpns_contract::OWNER_ID_BYTES.into(),
            revision: 0,
    ...
    fn identity_update_with_encryption_key_check_tx() {
        ...
        config.abci.keys.hellarpay_master_public_key = master_public_key.serialize().to_vec();
        ...
        config.abci.keys.hellarpay_second_public_key = high_public_key.serialize().to_vec();
        ...
        let update_transition: IdentityUpdateTransition = IdentityUpdateTransitionV0 {
            identity_id: hellarpay_contract::OWNER_ID_BYTES.into(),
            revision: 1,
