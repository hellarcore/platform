use hpp::consensus::state::data_trigger::data_trigger_condition_error::DataTriggerConditionError;
use hpp::data_contract::accessors::v0::DataContractV0Getters;
use hpp::data_contracts::hpns_contract::document_types::domain::properties::PARENT_DOMAIN_NAME;
///! The `hpns_triggers` module contains data triggers specific to the HPNS data contract.
use hpp::util::hash::hash;
...
use hpp::platform_value::btreemap_extensions::{BTreeValueMapHelper, BTreeValueMapPathHelper};
...
use hpp::system_data_contracts::hpns_contract;
use hpp::system_data_contracts::hpns_contract::document_types::domain::properties::{ALLOW_SUBDOMAINS, HELLAR_ALIAS_IDENTITY_ID, HELLAR_UNIQUE_IDENTITY_ID, LABEL, NORMALIZED_LABEL, NORMALIZED_PARENT_DOMAIN_NAME, PREORDER_SALT, RECORDS};
use hpp::util::strings::convert_to_homograph_safe_chars;
...
/// * `hpns_contract::OWNER_ID` - An optional identifier for the top-level identity associated with the domain
...
pub fn create_domain_data_trigger_v0(
    ...
    if !is_dry_run {
        ...
        if let Some(id) = records
            .get_optional_identifier(HELLAR_UNIQUE_IDENTITY_ID)
            .map_err(ProtocolError::ValueError)?
        {
            if id != owner_id {
                let err = DataTriggerConditionError::new(
                    ...
                    format!(
                        ...
                        owner_id, HELLAR_UNIQUE_IDENTITY_ID, id
                    ),
                );
        ...
        if let Some(id) = records
            .get_optional_identifier(HELLAR_ALIAS_IDENTITY_ID)
            .map_err(ProtocolError::ValueError)?
        {
            if id != owner_id {
                let err = DataTriggerConditionError::new(
                    ...
                    format!(
                        "ownerId {} doesn't match {} {}",
                        owner_id, HELLAR_ALIAS_IDENTITY_ID, id
                    ),
                );
        ...
        if normalized_parent_domain_name.is_empty() && context.owner_id != &hpns_contract::OWNER_ID
        {
            let err = DataTriggerConditionError::new(
                data_contract.id(),
                document_transition.base().id(),
...
mod test {
    ...
    use hpp::tests::fixtures::{get_document_transitions_fixture, get_hpns_data_contract_fixture, get_hpns_parent_document_fixture, ParentDocumentOptions};
    use hpp::tests::utils::generate_random_identifier_struct;
    use hpp::version::{DefaultForPlatformVersion};
    ...
    fn should_return_execution_result_on_dry_run() {
        ...
        let document = get_hpns_parent_document_fixture(
        ...
        let data_contract = get_hpns_data_contract_fixture(
            Some(owner_id),
            state_read_guard.current_protocol_version_in_consensus(),
        )
        .data_contract_owned();
        ...
        let result = create_domain_data_trigger_v0(
            &DocumentCreateTransitionAction::from_document_borrowed_create_transition_with_contract_lookup(
                document_create_transition,|_identifier| {
                    Ok(Arc::new(DataContractFetchInfo::hpns_contract_fixture(platform_version.protocol_version)))
