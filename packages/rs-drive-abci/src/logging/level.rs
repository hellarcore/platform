...
impl TryFrom<&LogLevel> for EnvFilter {
    ...
    fn try_from(value: &LogLevel) -> Result<Self, Error> {
        let env_filter = match value {
            ...
            LogLevel::Warn => {
                EnvFilter::try_new("error,tenderhellar_abci=warn,drive_abci=warn,drive=warn,hpp=warn")
                    .expect("should be a valid log specification")
            }
            LogLevel::Info => {
                EnvFilter::try_new("error,tenderhellar_abci=info,drive_abci=info,drive=info,hpp=info")
                    .expect("should be a valid log specification")
            }
            LogLevel::Debug => EnvFilter::try_new(
                "info,tenderhellar_abci=debug,drive_abci=debug,drive=debug,hpp=debug",
            )
            .expect("should be a valid log specification"),
            LogLevel::Trace => EnvFilter::try_new(
                "debug,tenderhellar_abci=trace,drive_abci=trace,drive=trace,drive_grovedb_operations=off,hpp=trace",
            )
            .expect("should be a valid log specification"),
            LogLevel::Paranoid => {
