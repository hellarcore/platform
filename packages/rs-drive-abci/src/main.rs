...
//!
//! RS-Drive-ABCI server starts a single-threaded server and listens to connections from Tenderhellar.
...
/// Server that accepts connections from Tenderhellar, and
/// executes Hellar Platform logic as part of the ABCI++ protocol.
///
    ...
    fn run(self, config: PlatformConfig, cancel: CancellationToken) -> Result<(), String> {
        match self.command {
            Commands::Start => {
                ...
                // Drive and Tenderhellar rely on Core. Various functions will fail if Core is not synced.
                // We need to make sure that Core is ready before we start Drive ABCI app
                // Tenderhellar won't start too until ABCI port is open.
...
fn main() -> Result<(), ExitCode> {
    ...
    // The runtime will be reused by Prometheus and rs-tenderhellar-abci.
