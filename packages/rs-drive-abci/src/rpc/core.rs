use hellarcore_rpc::hellarcore::ephemerealdata::chain_lock::ChainLock;
use hellarcore_rpc::hellarcore::{Block, BlockHash, QuorumHash, Transaction, Txid};
use hellarcore_rpc::hellarcore_rpc_json::{
...
use hellarcore_rpc::json::GetRawTransactionResult;
use hellarcore_rpc::{Auth, Client, Error, RpcApi};
use hpp::hellarcore::{hashes::Hash, InstantLock};
...
pub trait CoreRPCLike {
    ...
    fn get_optional_transaction_extended_info(
        ...
        match self.get_transaction_extended_info(transaction_id) {
            ...
            Err(Error::JsonRpc(hellarcore_rpc::jsonrpc::error::Error::Rpc(
                hellarcore_rpc::jsonrpc::error::RpcError {
    ...
    ///
    /// See <https://hellarcore.readme.io/v19.0.0/docs/core-api-ref-remote-procedure-calls-evo#quorum-listextended>
    ...
    ///
    /// See <https://hellarcore.readme.io/v19.0.0/docs/core-api-ref-remote-procedure-calls-evo#quorum-info>
    fn get_quorum_info(
        &self,
...
#[derive(Debug)]
/// Default implementation of Hellar Core RPC using hellarcoreRPC client
    ...
    ($action:expr) => {{
        ...
        for i in 0..MAX_RETRIES {
            match $action {
                ...
                Err(e) => {
                    match e {
                        hellarcore_rpc::Error::JsonRpc(
                            // Retry on transport connection error
                            hellarcore_rpc::jsonrpc::error::Error::Transport(_)
                            | hellarcore_rpc::jsonrpc::error::Error::Rpc(
