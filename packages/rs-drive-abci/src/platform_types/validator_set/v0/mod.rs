...
use hellarcore_rpc::json::QuorumInfoResult;
use hpp::bls_signatures::PublicKey as BlsPublicKey;
...
use tenderhellar_abci::proto::{abci, crypto};
...
/// Reverse bytes
///
/// TODO: This is a workaround for reversed data returned by hellarcore_rpc (little endian / big endian handling issue).
            ...
            .filter_map(|quorum_member| {
                ...
                let public_key = if let Some(public_key_share) = quorum_member.pub_key_share {
                    match BlsPublicKey::from_bytes(public_key_share.as_slice())
                        .map_err(ExecutionError::BlsErrorFromhellarcoreResponse)
                    {
