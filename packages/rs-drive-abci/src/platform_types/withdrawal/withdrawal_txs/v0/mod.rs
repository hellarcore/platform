...
use tenderhellar_abci::signatures::SignDigest;
...
impl<'a> WithdrawalTxs<'a> {
    ...
    /// 2. Signature or public key is malformed - in this case, [AbciError::BlsErrorOfTenderhellarThresholdMechanism] is returned
    /// 3. Provided data is invalid - [AbciError::TenderhellarProto] is returned
    ///
    ...
    pub fn verify_signatures(
        ...
        for s in &self.inner {
            let hash = match s.sign_digest(
                ...
                Ok(h) => h,
                Err(e) => return SimpleValidationResult::new_with_error(AbciError::Tenderhellar(e)),
            };
...
mod test {
    use hellarcore_rpc::hellarcore_rpc_json::QuorumType;
    use hpp::bls_signatures;
    use tenderhellar_abci::proto::types::{VoteExtension, VoteExtensionType};
