...
mod tests {
    ...
    use data_contracts::SystemDataContract::Hellarpay;

    #[test]
    fn test_contract_serialization() {
        let platform_version = PlatformVersion::latest();
        let data_contract = load_system_data_contract(Hellarpay, platform_version.protocol_version)
            .expect("expected hellarpay contract");
        let platform_version = PlatformVersion::latest();
    ...
    fn test_contract_can_have_specialized_contract_encryption_decryption_keys() {
        let data_contract = get_hellarpay_contract_with_generalized_encryption_key_fixture(None, 1)
            .data_contract_owned();
        assert_eq!(
            data_contract
                .config()
    ...
    fn test_contract_document_type_can_have_specialized_contract_encryption_decryption_keys() {
        let data_contract = get_hellarpay_contract_fixture(None, 1).data_contract_owned();
        assert_eq!(
            data_contract
