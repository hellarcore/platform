...
mod test {
    ...
    use crate::tests::fixtures::get_hellarpay_contract_fixture;
    ...
    pub(crate) fn data_contract_with_dynamic_properties() -> DataContract {
        ...
        let test_document_properties_alpha_identifier = Value::from([
            ...
            (
                "contentMediaType",
                Value::Text("application/x.hellar.hpp.identifier".to_string()),
            ),
        ]);
    ...
    fn test_document_json_deserialize() -> Result<()> {
        ...
        let document_json = get_data_from_file("src/tests/payloads/document_hpns.json")?;
        let doc =
            ExtendedDocument::from_json_string(&document_json, hpns_contract, platform_version)?;
        ...
        println!(
            "{:?}",
            doc.properties()
                .get_at_path("records.hellarUniqueIdentityId")
    ...
    fn test_to_object() {
        init();
        let hpns_contract = load_system_data_contract(
            SystemDataContract::HPNS,
            LATEST_PLATFORM_VERSION.protocol_version,
        )
        .unwrap();
        let document_json = get_data_from_file("src/tests/payloads/document_hpns.json").unwrap();
    ...
    fn test_document_to_buffer() -> Result<()> {
        ...
        .unwrap();
        ExtendedDocument::from_json_string(&document_json, hpns_contract, LATEST_PLATFORM_VERSION)
    ...
    fn new_example_document() -> ExtendedDocument {
        let data_contract =
            get_hellarpay_contract_fixture(None, LATEST_PLATFORM_VERSION.protocol_version)
                .data_contract_owned();
