...
use crate::{
    CompatibleProtocolVersionIsNotDefinedError, HellarPlatformProtocolInitError,
    InvalidVectorSizeError, NonConsensusError, SerdeParsingError,
};

use hellarcore::consensus::encode::Error as hellarcoreError;
...
pub enum ProtocolError {
    ...
    #[error(
    "hpp received not allowed version on {method}, allowed versions: {allowed_versions:?}, received: {received}"
    )]
    ...
    /// Platform expected some specific versions
    #[error(
        "hpp unknown version on {method}, known versions: {known_versions:?}, received: {received}"
    )]
    ...
    /// Hellar core error
    #[error("hellar core error: {0}")]
    hellarcoreError(#[from] hellarcoreError),
...
impl From<HellarPlatformProtocolInitError> for ProtocolError {
    fn from(e: HellarPlatformProtocolInitError) -> Self {
