...
class DocumentsBenchmark extends AbstractBenchmark {
  ...
  createMochaTestSuite(context) {
    ...
    suite.beforeAll('Publish Data Contract', async () => {
      const dataContract = await context.hellar.platform.contracts.create(
        documentTypes,
        context.identity,
      );
      ...
      await context.hellar.platform.contracts.publish(
        dataContract,
        context.identity,
      );

      context.hellar.getApps().set(this.config.title, {
    ...
    for (const documentType of Object.keys(documentTypes)) {
      ...
      for (let i = 0; i < this.config.documentsCount; i++) {
        suite.addTest(new Test(`Create document ${documentType}`, async () => {
          ...
          const document = await context.hellar.platform.documents.create(
          ...
          const stateTransition = await context.hellar.platform.documents.broadcast({
            create: [document],
          }, context.identity);
