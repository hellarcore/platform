//! [Sdk] entrypoint to Hellar Platform.
...
#[cfg(feature = "mocks")]
use crate::mock::MockHellarPlatformSdk;
use crate::mock::{MockRequest, MockResponse};
use crate::{core::CoreClient, error::Error};
use hpp::version::{PlatformVersion, PlatformVersionCurrentVersion};
#[cfg(feature = "mocks")]
...
pub use http::Uri;
#[cfg(feature = "mocks")]
use rs_hapi_client::mock::MockDapiClient;
pub use rs_hapi_client::AddressList;
use rs_hapi_client::{
...
/// Hellar Platform SDK
///
/// This is the main entry point for interacting with Hellar Platform.
/// It can be initialized in two modes:
/// - `Normal`: Connects to a remote Hellar Platform node.
/// - `Mock`: Uses a mock implementation of Hellar Platform.
///
...
enum SdkInstance {
    /// Real Sdk, using HAPI with gRPC transport
    Dapi {
        /// HAPI client used to communicate with Hellar Platform.
        hapi: DapiClient,
    ...
    /// Mock SDK
    Mock {
        /// Mock HAPI client used to communicate with Hellar Platform.
        hapi: Arc<Mutex<MockDapiClient>>,
        /// Mock SDK implementation processing mock expectations and responses.
        mock: MockHellarPlatformSdk,
        quorum_provider: MockQuorumInfoProvider,
    },
}

impl Sdk {
    /// Initialize Hellar Platform  SDK in mock mode.
    ///
    ...
    ///
    /// This method is used to retrieve objects from proofs returned by Hellar Platform.
    ///
    /// ## Generic Parameters
    ///
    ...
    /// Returns a mutable reference to the `MockHellarPlatformSdk` instance.
    ///
    ...
    #[cfg(feature = "mocks")]
    pub fn mock(&mut self) -> &mut MockHellarPlatformSdk {
        if let Sdk {
    ...
    /// Return [Hellar Platform version](PlatformVersion) information used by this SDK.
    ///
    ///
    ///
...
impl Dapi for Sdk {
    async fn execute<R: TransportRequest>(
        ...
        match self.inner {
            SdkInstance::Dapi { ref mut hapi, .. } => hapi.execute(request, settings).await,
            #[cfg(feature = "mocks")]
            SdkInstance::Mock { ref mut hapi, .. } => {
                let mut hapi_guard = hapi.lock().await;
                hapi_guard.execute(request, settings).await
            }
        }
    }
}

/// Hellar Platform SDK Builder, used to configure and [`SdkBuilder::build()`] the [Sdk].
///
...
impl SdkBuilder {
    ...
    /// Use this method if you want to connect to Hellar Platform testnet during development and testing
    /// of your solution.
    ...
    /// Use this method if you want to connect to Hellar Platform mainnet with production-ready product.
    ...
    ///
    /// Tune request settings used to connect to the Hellar Platform.
    ///
    ...
    ///
    /// Select specific version of Hellar Platform to use.
    ///
    ...
    /// Configure connection to Hellar Core
    ///
    ...
    ///
    /// These files can be used together with [MockHellarPlatformSdk] to replay the requests and responses.
    /// See [MockHellarPlatformSdk::load_expectations()] for more information.
    ///
    ...
    pub fn build(self) -> Result<Sdk, Error> {
        ...
        match self.addresses {
            Some(addresses) => {
                ...
                let hapi = DapiClient::new(addresses, self.settings);
                #[cfg(feature = "mocks")]
                let hapi = hapi.dump_dir(self.dump_dir.clone());
                ...
                Ok(Sdk{
                    inner:SdkInstance::Dapi { hapi, core, version:self.version },
                    proofs:self.proofs,
                    #[cfg(feature = "mocks")]
            ...
            #[cfg(feature = "mocks")]
            None =>{ let hapi =Arc::new(Mutex::new(  MockDapiClient::new()));
                Ok(Sdk{
                    inner:SdkInstance::Mock {
                        mock: MockHellarPlatformSdk::new(self.version, Arc::clone(&hapi), self.proofs),
                        hapi,
