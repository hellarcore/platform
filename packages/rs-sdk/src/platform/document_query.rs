...
use hapi_grpc::platform::v0::get_documents_request::Version::V0;
...
/// Request that is used to query documents from the Hellar Platform.
///
/// This is an abstraction layer built on top of [GetDocumentsRequest] to address issues with missing details
/// required to correctly verify proofs returned by the Hellar Platform.
///
...
impl TransportRequest for DocumentQuery {
    ...
    const SETTINGS_OVERRIDES: rs_hapi_client::RequestSettings =
...
impl TryFrom<DocumentQuery> for platform_proto::GetDocumentsRequest {
    type Error = Error;
    fn try_from(hapi_request: DocumentQuery) -> Result<Self, Self::Error> {
        // TODO implement where and order_by clause

        let where_clauses = serialize_vec_to_cbor(hapi_request.where_clauses.clone())
            .expect("where clauses serialization should never fail");
        let order_by = serialize_vec_to_cbor(hapi_request.order_by_clauses.clone())?;
        // Order clause

        //todo: transform this into PlatformVersionedTryFrom
        Ok(GetDocumentsRequest {
            version: Some(V0(GetDocumentsRequestV0 {
                data_contract_id: hapi_request.data_contract.id().to_vec(),
                document_type: hapi_request.document_type_name.clone(),
                r#where: where_clauses,
                order_by,
                limit: hapi_request.limit,
                prove: true,
                start: hapi_request.start.clone(),
            })),
        })
    }
}
...
fn serialize_vec_to_cbor<T: Into<Value>>(input: Vec<T>) -> Result<Vec<u8>, Error> {
    ...
    let cbor_values: CborValue = TryInto::<CborValue>::try_into(values)
        .map_err(|e| Error::Protocol(hpp::ProtocolError::EncodingError(e.to_string())))?;
