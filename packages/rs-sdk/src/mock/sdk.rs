//! Mocking mechanisms for Hellar Platform SDK.
//!
//! See [MockHellarPlatformSdk] for more details.
...
/// Mechanisms to mock Hellar Platform SDK.
///
/// This object is returned by [Sdk::mock()](crate::Sdk::mock()) and is used to define mock expectations.
///
/// Use [MockHellarPlatformSdk::expect_fetch_many()] to define expectations for [FetchMany] requests
/// and [MockHellarPlatformSdk::expect_fetch()] for [Fetch] requests.
///
/// ## Panics
///
/// Can panic on errors.
pub struct MockHellarPlatformSdk {
...
impl MockHellarPlatformSdk {
    ...
    async fn load_expectation<T: TransportRequest + for<'de> Deserialize<'de> + MockRequest>(
        ...
        self.hapi.lock().await.expect(&data.request, &data.response);
        Ok(())
    }
    ...
    /// #
    /// # r.block_on(async {
    ///     use rs_sdk::{Sdk, platform::{Identity, Fetch, hpp::identity::accessors::IdentityGettersV0}};
    ///
    ...
    ///
    ///     // Fetch the identity
    ///     let retrieved = hpp::prelude::Identity::fetch(&mut api, query)
    ///         .await
    ///         .unwrap()
    ...
    /// [MockHellarPlatformSdk::expect_fetch()], but the expected
        ...
        I::Response: Default,
    {
        ...
        let mut hapi_guard = self.hapi.lock().await;
        // We don't really care about the response, as it will be mocked by from_proof
        hapi_guard.expect(&grpc_request, &Default::default());
    }
