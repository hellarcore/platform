...
use rs_hapi_client::mock::Key;
use serde::{Deserialize, Serialize};

use super::MockHellarPlatformSdk;
...
pub trait MockResponse {
    ...
    ///
    /// Can panic on errors.
    fn mock_serialize(&self, mock_sdk: &MockHellarPlatformSdk) -> Vec<u8>;
    ...
    ///
    /// Can panic on errors.
    fn mock_deserialize(mock_sdk: &MockHellarPlatformSdk, buf: &[u8]) -> Self
    where
        Self: Sized;
}
...
{
    fn mock_deserialize(_sdk: &MockHellarPlatformSdk, buf: &[u8]) -> Self
    where
        Self: Sized,
    {
    ...
    fn mock_serialize(&self, _sdk: &MockHellarPlatformSdk) -> Vec<u8> {
...
{
    fn mock_serialize(&self, sdk: &MockHellarPlatformSdk) -> Vec<u8> {
    ...
    fn mock_deserialize(sdk: &MockHellarPlatformSdk, buf: &[u8]) -> Self
    where
