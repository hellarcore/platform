...
use hellarcore_rpc::{
    hellarcore::{hashes::Hash, QuorumHash},
    hellarcore_rpc_json::QuorumType,
};
...
impl CoreClient {
    /// Create new Hellar Core client.
    ///
    /// # Arguments
    ///
    /// * `server_address` - Hellar Core server address.
    /// * `core_port` - Hellar Core port.
    /// * `core_user` - Hellar Core user.
    /// * `core_password` - Hellar Core password.
    pub fn new(
        server_address: &str,
        core_port: u16,
        core_user: &str,
...
impl QuorumInfoProvider for CoreClient {
    fn get_quorum_public_key(
        ...
        let quorum_info = core
            ...
            .map_err(
                |e: hellarcore_rpc::Error| drive_proof_verifier::Error::InvalidQuorum {
                    error: e.to_string(),
                },
            )?;
