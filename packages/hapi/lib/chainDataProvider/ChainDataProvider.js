const ChainLockSigMessage = require('@hellarpro/hellarcore-lib/lib/zmqMessages/ChainLockSigMessage');
const { EventEmitter } = require('events');
const { BlockHeader, ChainLock } = require('@hellarpro/hellarcore-lib');
...
class ChainDataProvider extends EventEmitter {
  ...
  async init() {
    ...
    try {
      ...
      if (e.code === -32603) {
        logger.info('No chain lock available in hellarcore node');
      } else {
        throw e;
      }
    }
  ...
  async getBlockHeaders(fromHash, fromHeight, count) {
    ...
    // otherwise, re-fetch all headers from hellarcore
    if (firstCachedItem) {
      ...
      if (firstMissingIndex !== -1) {
        ...
        // to fetch from hellarcore only missing items
