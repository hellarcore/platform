const {
  startDapi,
} = require('@hellarpro/dp-services-ctl');
...
describe.skip('subscribeToTransactionsWithProofsHandlerFactory', function main() {
  ...
  beforeEach(async () => {
    ...
    const {
      ...
      hapi: {
        ...
        container: {
          volumes: [
            `${process.cwd()}/lib:/platform/packages/hapi/lib`,
            `${process.cwd()}/scripts:/platform/packages/hapi/scripts`,
          ],
        },
      },
    });
    ...
    coreAPI = hellarcore.getApi();
    hapiClient = hapiTxFilterStream.getApi();
  ...
  it('should respond with only historical data', async () => {
    ...
    const stream = await hapiClient.core.subscribeToTransactionsWithProofs(
      {
