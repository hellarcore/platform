const { BlockHeader, Block, ChainLock } = require('@hellarpro/hellarcore-lib');
...
describe('subscribeToNewBlockHeaders', () => {
  ...
  beforeEach(async function beforeEach() {
    ...
    this.sinon.stub(hellarcoreRpcClient, 'getBlockHeader')
      .callsFake(async (hash) => blockHeaders[hash].toBuffer().toString('hex'));

    this.sinon.stub(hellarcoreRpcClient, 'getBestChainLock').resolves({
      height: 1,
      signature: Buffer.from('fakeSig'),
      blockHash: Buffer.from('fakeHash'),
    });

    this.sinon.stub(hellarcoreRpcClient, 'getBestBlockHeight').resolves(100);

    zmqClient = new ZmqClient();

    blockHeadersCache = new BlockHeadersCache();

    chainDataProvider = new ChainDataProvider(hellarcoreRpcClient, zmqClient, blockHeadersCache);
    await chainDataProvider.init();

    hellarcoreRpcClient.getBlockHeader.resetHistory();
  ...
  it('should use cache when historical data is sent', async () => {
    ...
    expect(hellarcoreRpcClient.getBlockHeader.callCount).to.be.equal(3);
    hellarcoreRpcClient.getBlockHeader.resetHistory();
    ...
    expect(hellarcoreRpcClient.getBlockHeader.callCount).to.be.equal(0);
  });
  ...
  it('should update chain height on new block', async () => {
    ...
    hellarcoreRpcClient.getBestBlockHeight.resolves(newChainHeight);
