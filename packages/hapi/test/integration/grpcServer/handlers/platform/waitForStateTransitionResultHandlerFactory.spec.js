...
const getIdentityCreateTransitionFixture = require('@hellarpro/wasm-hpp/lib/test/fixtures/getIdentityCreateTransitionFixture');
const { default: loadWasmDpp, HellarPlatformProtocol } = require('@hellarpro/wasm-hpp');

const { EventEmitter } = require('events');

const cbor = require('cbor');
const NotFoundGrpcError = require('@hellarpro/grpc-common/lib/server/error/NotFoundGrpcError');
...
const waitForTransactionToBeProvableFactory = require('../../../../../lib/externalApis/tenderhellar/waitForTransactionToBeProvable/waitForTransactionToBeProvableFactory');
const waitForTransactionResult = require('../../../../../lib/externalApis/tenderhellar/waitForTransactionToBeProvable/waitForTransactionResult');
...
describe('waitForStateTransitionResultHandlerFactory', () => {
  ...
  beforeEach(async function beforeEach() {
    ...
    tenderHellarWsClientMock = new EventEmitter();
    tenderHellarWsClientMock.subscribe = this.sinon.stub();
    ...
    blockchainListener = new BlockchainListener(tenderHellarWsClientMock);
  ...
  it('should wait for state transition empty result', async () => {
    ...
    setTimeout(() => {
      tenderHellarWsClientMock.emit('tm.event = \'Tx\'', wsMessagesFixture.success);
    }, 10);
    setTimeout(() => {
      tenderHellarWsClientMock.emit(BlockchainListener.NEW_BLOCK_QUERY, {
  ...
  it('should wait for state transition and return result with error', (done) => {
    ...
    process.nextTick(() => {
      tenderHellarWsClientMock.emit('tm.event = \'Tx\'', wsMessagesFixture.error);
    });
  });
