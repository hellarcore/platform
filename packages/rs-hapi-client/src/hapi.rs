...
/// General HAPI request error type.
...
/// HAPI client trait.
pub trait Dapi {
    /// Execute request using this HAPI client.
    async fn execute<R>(
        &mut self,
        request: R,
...
impl Dapi for DapiClient {
        ...
        R: TransportRequest,
    {
        ...
        // Setup HAPI request execution routine future. It's a closure that will be called
        ...
        let routine = move || {
            ...
            // Get a transport client requried by the HAPI request from this HAPI client.
