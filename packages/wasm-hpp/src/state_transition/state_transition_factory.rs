...
use hpp::state_transition::errors::StateTransitionError;
...
impl StateTransitionFactoryWasm {
    pub fn state_transition_wasm_from_factory_result(
        ...
        match result {
            ...
            Err(hpp::ProtocolError::StateTransitionError(e)) => match e {
            ...
            Err(other) => Err(from_hpp_err(other)),
        }
    }
}
...
impl StateTransitionFactoryWasm {
    ...
    //
    //     let pk_validator =
    //         Arc::new(PublicKeysValidator::new(adapter.clone()).map_err(from_hpp_init_error)?);
