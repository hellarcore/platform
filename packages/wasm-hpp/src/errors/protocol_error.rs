...
pub fn from_protocol_error(protocol_error: hpp::ProtocolError) -> JsValue {
    match protocol_error {
        hpp::ProtocolError::ConsensusError(consensus_error) => {
            from_consensus_error(*consensus_error)
        }
        hpp::ProtocolError::InvalidDataContractError(err) => {
        ...
        hpp::ProtocolError::Error(anyhow_error) => {
