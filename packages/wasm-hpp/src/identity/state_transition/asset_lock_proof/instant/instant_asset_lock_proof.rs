...
use hpp::hellarcore::consensus::Encodable;
use hpp::identity::state_transition::asset_lock_proof::AssetLockProofType;
...
use crate::{
    buffer::Buffer,
    errors::{from_hpp_err, RustConversionError},
    identifier::IdentifierWrapper,
    with_js_error,
};
use hpp::identity::state_transition::asset_lock_proof::instant::{
    InstantAssetLockProof, RawInstantLockProof,
};
use hpp::platform_value::string_encoding;
...
impl InstantAssetLockProofWasm {
    ...
    pub fn get_output(&self) -> Result<JsValue, JsValue> {
        ...
        let output_json_string =
            serde_json::to_string(&TxOutSerdeHelper(output)).map_err(|e| from_hpp_err(e.into()))?;
