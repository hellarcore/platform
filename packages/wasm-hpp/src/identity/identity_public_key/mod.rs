...
use hpp::identity::identity_public_key::hash::IdentityPublicKeyHashMethodsV0;
use hpp::identity::{IdentityPublicKey, KeyID, TimestampMillis};
use hpp::platform_value::{BinaryData, ReplacementType, Value};
use hpp::serialization::ValueConvertible;
use hpp::state_transition::public_key_in_creation::v0::BINARY_DATA_FIELDS;
...
impl IdentityPublicKeyWasm {
    ...
    pub fn to_json(&self) -> Result<JsValue, JsValue> {
        let mut value = self.0.to_object().map_err(from_hpp_err)?;

        value
            .replace_at_paths(
                hpp::identity::identity_public_key::BINARY_DATA_FIELDS,
    ...
    pub fn to_object(&self) -> Result<JsValue, JsValue> {
        let value = self.0.to_object().map_err(from_hpp_err)?;
...
//         let val = serde_json::from_str(&str).map_err(|e| from_hpp_err(e.into()))?;
