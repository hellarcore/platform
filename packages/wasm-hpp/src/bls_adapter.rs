use anyhow::anyhow;
use hpp::{BlsModule, PublicKeyValidationError};
...
impl BlsModule for BlsAdapter {
    ...
    fn verify_signature(
        ...
        data: &[u8],
        public_key: &[u8],
    ) -> Result<bool, hpp::ProtocolError> {
        self.0
            .verifySignature(signature, data, public_key)
            .map_err(|_v| anyhow!("Can't verify signature").into())
    }

    fn private_key_to_public_key(&self, private_key: &[u8]) -> Result<Vec<u8>, hpp::ProtocolError> {
        self.0
    ...
    fn sign(&self, data: &[u8], private_key: &[u8]) -> Result<Vec<u8>, hpp::ProtocolError> {
        self.0
