...
case ${i} in
    ...
    ;;
    --hpns-tld-identity-private-key=*)
    identity_private_key="${i#*=}"
    ;;
    --hpns-tld-identity-id=*)
    tld_identity_id="${i#*=}"
    ;;
    --hpns-contract-id=*)
    tld_contract_id="${i#*=}"
  ...
  for scope in "${scopes[@]}"
    do
      case $scope in
        ...
        ;;
        e2e:hpns)
          scope_dirs="${scope_dirs} test/e2e/hpns.spec.js"
        ;;
        e2e:contacts)
...
then
  cmd="${cmd} HPNS_CONTRACT_ID=${tld_contract_id}"
fi

if [ -n "$tld_identity_id" ]
then
  cmd="${cmd} HPNS_TOP_LEVEL_IDENTITY_ID=${tld_identity_id}"
fi
...
then
  cmd="${cmd} HPNS_TOP_LEVEL_IDENTITY_PRIVATE_KEY=${identity_private_key}"
fi
