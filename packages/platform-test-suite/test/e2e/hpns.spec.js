...
describe('HPNS', () => {
  ...
  describe('Data contract', () => {
    it('should exists', async () => {
      const createdDataContract = await client.platform.contracts.get(hpnsContractId);

      expect(createdDataContract).to.exist();
      expect(createdDataContract.getId().toString()).to.equal(hpnsContractId);
    });
  });
  ...
  describe('HPNS owner', () => {
    ...
    before(async () => {
      ownerClient = await createClientWithFundedWallet(
        10000,
        process.env.HPNS_OWNER_PRIVATE_KEY,
      );

      newTopLevelDomain = getRandomDomain();
      identity = await ownerClient.platform.identities.get(hpnsOwnerId);

      expect(identity).to.exist();
      await ownerClient.platform.identities.topUp(hpnsOwnerId, 500);
    });
    ...
    //   And other is in hpp data trigger (old). New error is dispatched in this test,
    //   but it does not have binding yet.
    //   Decide what to do with rs-hpp data trigger and it's errors,
  ...
  describe('Any Identity', () => {
    ...
    //   (console.log error in hapi-client's GrpcTransport for more details)
    ...
    it.skip('should be able to register a second level domain', async () => {
      registeredDomain = await client.platform.names.register(`${secondLevelDomain}0.${topLevelDomain}`, {
        hellarUniqueIdentityId: identity.getId(),
      }, identity);
    ...
    it.skip('should be able to resolve domain by it\'s record', async () => {
      const [document] = await client.platform.names.resolveByRecord(
        ...
        registeredDomain.getData().records.hellarUniqueIdentityId,
      );
    ...
    it('should not be able to register two domains with same `hellarAliasIdentityId` record');

    it('should be able to register many domains with same `hellarAliasIdentityId` record');
