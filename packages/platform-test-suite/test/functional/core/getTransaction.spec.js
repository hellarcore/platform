...
const {
  Core: { Transaction, PrivateKey }, HAPIClient: {
    Errors: {
      NotFoundError,
    },
  },
...
describe('Core', () => {
  describe('getTransaction', () => {
    ...
    it('should respond with a transaction by it\'s ID', async () => {
      ...
      const result = await client.getHAPIClient().core.getTransaction(transaction.id);
    ...
    it('should throw NotFound error if transaction was not found', async () => {
      ...
      try {
        await client.getHAPIClient().core.getTransaction(nonExistentId);
