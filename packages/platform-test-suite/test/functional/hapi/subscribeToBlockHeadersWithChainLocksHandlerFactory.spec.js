...
const createRetryableStream = (hapiClient) => {
  ...
  const createStream = async (fromBlockHeight, count = 0) => {
    let streamError;
    const stream = await hapiClient.core.subscribeToBlockHeadersWithChainLocks(
      {
        fromBlockHeight,
        count,
      },
...
describe('subscribeToBlockHeadersWithChainLocksHandlerFactory', () => {
  ...
  beforeEach(async () => {
    ...
    bestBlock = new Block(
      await hapiClient.core.getBlockByHash(bestBlockHash),
    );
  ...
  it('should respond with only historical data', async () => {
    ...
    const stream = createRetryableStream(hapiClient);
    ...
    // TODO: fetching blocks one by one takes too long. Implement getBlockHeaders in hapi-client
    const fetchedBlocks = [];

    for (let i = 1; i <= headersAmount; i++) {
      const rawBlock = await hapiClient.core.getBlockByHeight(i);
