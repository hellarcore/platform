const featureFlagsSystemIds = require('@hellarpro/feature-flags-contract/lib/systemIds');
const InvalidRequestError = require('@hellarpro/hapi-client/lib/transport/errors/response/InvalidRequestError');
  ...
  describe('Feature flags', function main() {
    ...
    describe('updateConsensusParams', () => {
      ...
      before(async () => {
        ...
        oldConsensusParams = await ownerClient.getHAPIClient().platform.getConsensusParams();
      ...
      it('should update consensus params', async function it() {
        ...
        for (let i = 0; i < 5; i++) {
          try {
            newConsensusParams = await ownerClient.getHAPIClient()
              .platform
              .getConsensusParams(
