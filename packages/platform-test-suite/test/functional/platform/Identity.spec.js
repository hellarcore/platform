...
const { createFakeInstantLock } = require('hellar/build/utils/createFakeIntantLock');

const { hash } = require('@hellarpro/wasm-hpp/lib/utils/hash');
const getDataContractFixture = require('../../../lib/test/fixtures/getDataContractFixture');
const createClientWithFundedWallet = require('../../../lib/test/createClientWithFundedWallet');
const getHAPISeeds = require('../../../lib/test/getHAPISeeds');
...
describe('Platform', () => {
  describe('Identity', function describeIdentity() {
    ...
    it('should fail to create an identity if instantLock is not valid', async () => {
      ...
      const assetLockProof = await client.platform.hpp.identity.createInstantAssetLockProof(
    ...
    it('should be able to get newly created identity by it\'s public key', async () => {
      const response = await client.getHAPIClient().platform.getIdentitiesByPublicKeyHashes(
      ...
      //   asset lock proof. We would also want to do the same in rs-hpp and wasm-hpp, but
    ...
    describe('chainLock', function describe() {
      ...
      it('should create identity using chainLock', async () => {
        ...
        const assetLockProof = await client.platform.hpp.identity.createChainAssetLockProof(
          transactionHeight,
    ...
    describe('Credits', () => {
      ...
      it('should fail top-up if instant lock is not valid', async () => {
        ...
        const assetLockProof = await client.platform.hpp.identity
        ...
        const identityTopUpTransition = client.platform.hpp.identity
    ...
    describe.skip('Masternodes', () => {
      ...
      beforeEach(() => {
        hapiClient = new Hellar.HAPIClient({
          network,
          seeds: getHAPISeeds(),
        });
      });
      ...
      it('should receive masternode identities', async () => {
        ...
        const bestBlockHash = await hapiClient.core.getBestBlockHash();
        const baseBlockHash = await hapiClient.core.getBlockHash(1);

        const { mnList } = await hapiClient.core.getMnListDiff(
          baseBlockHash,
          bestBlockHash,
        );
        ...
        for (const masternodeEntry of mnList) {
          ...
          const { transaction: transactionBuffer } = await client.hapiClient.core.getTransaction(
