...
const ResponseError = require('@hellarpro/hapi-client/lib/transport/errors/response/ResponseError');
const InvalidRequestDPPError = require('@hellarpro/hapi-client/lib/transport/errors/response/InvalidRequestDPPError');

const createGrpcTransportError = require('@hellarpro/hapi-client/lib/transport/GrpcTransport/createGrpcTransportError');
...
export default async function broadcastStateTransition(
  ...
  try {
    await client.getHAPIClient().platform.broadcastStateTransition(serializedStateTransition);
  } catch (error) {
  ...
  const stateTransitionResult: IStateTransitionResult = await client
    .getHAPIClient().platform.waitForStateTransitionResult(hash, { prove: true });

  const { error } = stateTransitionResult;

  if (error) {
    // Create HAPI response error from gRPC error passed as gRPC response
