...
// import { getLatestProtocolVersion } from '@hellarpro/wasm-hpp';
import { Platform } from './index';
import 'mocha';
import Client from '../Client';

describe('Hellar - Platform', () => {
  ...
  it.skip('should set protocol version for DPP though options', async () => {
    ...
    // expect(platform.hpp.getProtocolVersion()).to.equal(42);
  });
  ...
  it.skip('should set protocol version for DPP using mapping', async () => {
    ...
    await platform.initialize();
    // expect(platform.hpp.getProtocolVersion()).to.equal(testnetProtocolVersion);
  });
  ...
  it.skip('should set protocol version for DPP using latest version', async () => {
    ...
    await platform.initialize();
    // expect(platform.hpp.getProtocolVersion()).to.equal(latestProtocolVersion);
  });
});
