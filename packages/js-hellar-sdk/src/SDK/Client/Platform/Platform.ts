import loadWasmDpp, { HellarPlatformProtocol, getLatestProtocolVersion } from '@hellarpro/wasm-hpp';
import type { DPPModule } from '@hellarpro/wasm-hpp';
...
export class Platform {
  ...
  //  whether hpp is initialized instead of ts-ignoring?
  // @ts-ignore
  hpp: HellarPlatformProtocol;
  ...
  constructor(options: PlatformOpts) {
    ...
    this.fetcher = new Fetcher(this.client.getHAPIClient());
  }
  ...
  async initialize() {
    if (!this.hpp) {
      ...
      if (this.protocolVersion === undefined) {
        ...
        // fallback to one that set in hpp as the last option
      ...
      this.hpp = new HellarPlatformProtocol(
        {
  ...
  //
  // ```
  // static initializeDppModule(): Promise<typeof import("@hellarpro/wasm-hppdist/hpp")>;
  // ```
  //
