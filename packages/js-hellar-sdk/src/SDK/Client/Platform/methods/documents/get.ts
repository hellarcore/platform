import { Identifier, ExtendedDocument, Metadata } from '@hellarpro/wasm-hpp';
...
export async function get(this: Platform, typeLocator: string, opts: QueryOptions): Promise<any> {
  ...
  // locator is of `hellarpay.profile` with hellarpay the app and profile the field.
  ...
  try {
    documentsResponse = await this.client.getHAPIClient().platform.getDocuments(
  ...
  const result = await Promise.all(
    rawDocuments.map(async (rawDocument) => {
      const document = await this.hpp.document
