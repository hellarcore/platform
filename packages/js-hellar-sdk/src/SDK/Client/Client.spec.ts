...
import { Transaction, BlockHeader, PrivateKey } from '@hellarpro/hellarcore-lib';
import { IdentityPublicKey, IdentityPublicKeyWithWitness, StateTransitionTypes } from '@hellarpro/wasm-hpp';
...
const getDocumentsFixture = require('@hellarpro/wasm-hpp/lib/test/fixtures/getDocumentsFixture');
...
describe('Hellar - Client', function suite() {
  this.timeout(30000);
  ...
  it('should be instantiable', () => {
    ...
    expect(client.getHAPIClient().constructor.name).to.be.equal('HAPIClient');
  });
  ...
  describe('#platform.identities.register', async () => {
    it('should register an identity', async () => {
      ...
      const serializedSt = hapiClientMock.platform.broadcastStateTransition.getCall(0).args[0];
      const interceptedIdentityStateTransition = await client
        .platform.hpp.stateTransition.createFromBuffer(serializedSt);
    ...
    it('should throw TransitionBroadcastError when transport resolves error', async () => {
      ...
      hapiClientMock.platform.waitForStateTransitionResult.resolves(errorResponse);
  ...
  describe('#platform.identities.topUp', async () => {
    it('should top up an identity', async () => {
      ...
      const serializedSt = hapiClientMock.platform.broadcastStateTransition.getCall(1).args[0];
      const interceptedIdentityStateTransition = await client
        .platform.hpp.stateTransition.createFromBuffer(serializedSt);
      const interceptedAssetLockProof = interceptedIdentityStateTransition.getAssetLockProof();
  ...
  describe('#platform.documents.broadcast', () => {
    ...
    it('should broadcast documents', async () => {
      ...
      hapiClientMock.platform.waitForStateTransitionResult.resolves(proofResponse);
  ...
  describe('#platform.contracts.publish', () => {
    ...
    it('should broadcast data contract', async () => {
      hapiClientMock.platform.waitForStateTransitionResult.resolves({
        proof: { },
      });
