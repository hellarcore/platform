...
import generateTenderhellarNodeKey from '../../src/tenderhellar/generateTenderhellarNodeKey.js';
...
describe('Testnet Evonode', function main() {
  ...
  describe('setup', () => {
    it('should setup fullnode', async () => {
      ...
      await setupTask.run({
        ...
        tenderhellarNodeKey: generateTenderhellarNodeKey(),
        initialIpForm: {
          ip,
          coreP2PPort: 19999,
      ...
      if (process.env.HELLARMATE_E2E_TESTS_SKIP_IMAGE_BUILD !== 'true') {
        config.set('hellarmate.helper.docker.build.enabled', true);
        config.set('platform.drive.abci.docker.build.enabled', true);
        config.set('platform.hapi.api.docker.build.enabled', true);
      }

      config.set('docker.network.subnet', '172.27.24.0/24');
      config.set('hellarmate.helper.api.port', 40000);
      config.set('core.p2p.port', 40001);
      config.set('core.rpc.port', 40002);
      config.set('platform.hapi.envoy.http.port', 40003);
      config.set('platform.drive.tenderhellar.p2p.port', 40004);
      config.set('platform.drive.tenderhellar.rpc.port', 40005);
      config.set('platform.drive.tenderhellar.pprof.port', 40006);
      config.set('hellarmate.helper.api.port', 41337);
  ...
  describe('sync', () => {
    it('should sync Hellar Core', async () => {
  ...
  describe('restart', () => {
    it('should restart fullnode and continue syncing Hellar Core', async () => {
  ...
  describe('hellarmate helper', () => {
    it('should be able to request HTTP to helper api', async () => {
      await assertServiceRunning(config, 'hellarmate_helper', true);

      // wait for http api to come up
      await waitForHellarmateHelperAPI(config);

      const response = await fetch(`http://127.0.0.1:${config.get('hellarmate.helper.api.port')}`, {
      ...
      expect(scope.platform.tenderhellar.dockerStatus).to.be.equal(DockerStatusEnum.running);
      expect(scope.platform.tenderhellar.serviceStatus).to.be.equal(ServiceStatusEnum.wait_for_core);
    });
  });
