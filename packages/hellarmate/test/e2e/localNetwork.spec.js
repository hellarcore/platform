...
describe('Local Network', function main() {
  ...
  before(async () => {
    ...
    if (process.env.HELLARMATE_E2E_TESTS_LOCAL_HOMEDIR) {
      homeDir.change(new HomeDir(process.env.HELLARMATE_E2E_TESTS_LOCAL_HOMEDIR));
    } else {
    ...
    if (process.env.HELLARMATE_E2E_TESTS_SKIP_IMAGE_BUILD !== 'true') {
      localConfig.set('hellarmate.helper.docker.build.enabled', true);
      localConfig.set('platform.drive.abci.docker.build.enabled', true);
      localConfig.set('platform.hapi.api.docker.build.enabled', true);
    }

    localConfig.set('docker.network.subnet', '172.30.0.0/24');
    localConfig.set('hellarmate.helper.api.port', 40000);
    localConfig.set('core.p2p.port', 40001);
    localConfig.set('core.rpc.port', 40002);
    localConfig.set('platform.hapi.envoy.http.port', 40003);
    localConfig.set('platform.drive.tenderhellar.p2p.port', 40004);
    localConfig.set('platform.drive.tenderhellar.rpc.port', 40005);
    localConfig.set('platform.drive.tenderhellar.pprof.port', 40006);
