...
describe('Config set command', () => {
  ...
  describe('#platform', () => {
    ...
    it('should allow setting booleans', async () => {
      ...
      await command.runWithDependencies({
        option: 'hellarmate.helper.api.enable', value: 'true',
      }, flags, config);

      expect(config.get('hellarmate.helper.api.enable')).to.equal(true);
    ...
    it('should throw on type mismatch', async () => {
      ...
      try {
        await command.runWithDependencies({
          option: 'hellarmate.helper.api.enable', value: 120,
        }, flags, config);
