...
describe('getPlatformScopeFactory', () => {
  describe('#getPlatformScope', () => {
    ...
    beforeEach(async function it() {
      ...
      httpPort = config.get('platform.hapi.envoy.http.port');
      httpService = `${config.get('externalIp')}:${httpPort}`;
      p2pPort = config.get('platform.drive.tenderhellar.p2p.port');
      p2pService = `${config.get('externalIp')}:${p2pPort}`;
      rpcPort = config.get('platform.drive.tenderhellar.rpc.port');
    ...
    it('should return empty scope if error during request to core', async () => {
      ...
      mockDockerCompose.execCommand.returns({ exitCode: 0, out: '' });
      mockDockerCompose.isServiceRunning.returns(true);
      mockDetermineDockerStatus.withArgs(mockDockerCompose, config, 'drive_tenderhellar')
    ...
    it('should return empty scope if core is not synced', async () => {
      mockDockerCompose.isServiceRunning
        .withArgs(config, 'drive_tenderhellar')
        .returns(true);
      mockDetermineDockerStatus.withArgs(mockDockerCompose, config, 'drive_tenderhellar').returns(DockerStatusEnum.running);
    ...
    it('should return drive info if tenderhellar is failed', async () => {
      mockRpcClient.mnsync.withArgs('status').returns({ result: { IsSynced: true } });
      mockDockerCompose.isServiceRunning
        .withArgs(config, 'drive_tenderhellar')
        .returns(true);
    ...
    it('should still return scope with tenderhellar if drive is failed', async () => {
      mockRpcClient.mnsync.withArgs('status').returns({ result: { IsSynced: true } });
      mockDockerCompose.isServiceRunning
        .withArgs(config, 'drive_tenderhellar')
        .returns(true);
    ...
    it('should have error service status in case FetchError to tenderhellar', async () => {
