  ...
  function startNodeTask(config) {
    ...
    if (config.get('platform.enable')) {
      ...
      if (tenderhellarLogFilePath !== null) {
        ensureFileMountExists(tenderhellarLogFilePath, 0o666);
      }
    }
      ...
      },
      {
        ...
        task: async () => {
          if (!await dockerCompose.isServiceRunning(config, 'core')) {
            throw new Error('Platform services depend on Core and can\'t be started without it. Please run "hellarmate start" without "--platform" flag');
          }
        },
      },
      {
