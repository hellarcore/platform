 ...
 * @param {configureTenderhellarTask} configureTenderhellarTask
    ...
    return new Listr([
      ...
      },
      {
        ...
        task: async (ctx, task) => {
          ...
          const {
            hdPrivateKey: hpnsPrivateKey,
            derivedPrivateKeys: [
              hpnsDerivedMasterPrivateKey,
              hpnsDerivedSecondPrivateKey,
            ],
          ...
          const {
            hdPrivateKey: hellarpayPrivateKey,
            derivedPrivateKeys: [
              hellarpayDerivedMasterPrivateKey,
              hellarpayDerivedSecondPrivateKey,
            ],
          } = await generateHDPrivateKeys(network, [0, 1]);
          ...
          task.output = `HPNS Private Key: ${hpnsPrivateKey.toString()}`;

          // eslint-disable-next-line no-param-reassign
          task.output = `Feature Flags Private Key: ${featureFlagsPrivateKey.toString()}`;

          // eslint-disable-next-line no-param-reassign
          task.output = `Hellarpay Private Key: ${hellarpayPrivateKey.toString()}`;
          ...
          const subTasks = ctx.configGroup.map((config, i) => (
            {
              ...
              task: () => {
                ...
                config.set('hellarmate.helper.api.port', config.get('hellarmate.helper.api.port') + (i * 100));
                ...
                if (config.getName() === 'local_seed') {
                  ...
                  config.set('platform.drive.tenderhellar.mode', 'seed');
                } else {
                  config.set('description', `local node #${nodeIndex}`);

                  const key = generateTenderhellarNodeKey();
                  const id = deriveTenderhellarNodeId(key);

                  config.set('platform.drive.tenderhellar.node.id', id);
                  config.set('platform.drive.tenderhellar.node.key', key);

                  config.set('platform.hapi.envoy.http.port', config.get('platform.hapi.envoy.http.port') + (i * 100));
                  config.set('platform.drive.tenderhellar.p2p.port', config.get('platform.drive.tenderhellar.p2p.port') + (i * 100));
                  config.set('platform.drive.tenderhellar.rpc.port', config.get('platform.drive.tenderhellar.rpc.port') + (i * 100));
                  config.set('platform.drive.tenderhellar.pprof.port', config.get('platform.drive.tenderhellar.pprof.port') + (i * 100));
                  config.set('platform.drive.tenderhellar.metrics.port', config.get('platform.drive.tenderhellar.metrics.port') + (i * 100));
                  config.set('platform.drive.tenderhellar.moniker', config.name);

                  config.set('platform.drive.tenderhellar.mode', 'validator');
                  ...
                  if (ctx.debugLogs) {
                    ...
                    config.set('platform.drive.tenderhellar.log.level', 'debug');
                  }

                  config.set('platform.hpns.masterPublicKey', hpnsDerivedMasterPrivateKey.privateKey.toPublicKey().toString());
                  config.set('platform.hpns.secondPublicKey', hpnsDerivedSecondPrivateKey.privateKey.toPublicKey().toString());
                  ...
                  config.set('platform.hellarpay.masterPublicKey', hellarpayDerivedMasterPrivateKey.privateKey.toPublicKey().toString());
                  config.set('platform.hellarpay.secondPublicKey', hellarpayDerivedSecondPrivateKey.privateKey.toPublicKey().toString());
      ...
      {
        title: 'Configure Tenderhellar nodes',
        task: (ctx) => configureTenderhellarTask(ctx.configGroup),
      },
      {
