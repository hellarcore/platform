...
export default function setupRegularPresetTaskFactory(
            ...
            nodeTypeName = await task.prompt([
              {
                ...
                header: `  The Hellar network consists of several different node types:
      Fullnode             - Host the full Hellar blockchain (no collateral)
      Masternode           - Fullnode features, plus Core services such as ChainLocks
                            and InstantSend (1000 HELLAR collateral)
      Evolution fullnode   - Fullnode features, plus host a full copy of the Platform
                            blockchain (no collateral)
      Evolution masternode - Masternode features, plus Platform services such as HAPI
                            and Drive (4000 HELLAR collateral)\n`,
                message: 'Select node type',
                choices: [
                  { name: NODE_TYPE_NAMES.FULLNODE },
                  { name: NODE_TYPE_NAMES.MASTERNODE, hint: '1000 HELLAR collateral' },
                  { name: NODE_TYPE_NAMES.HP_FULLNODE },
                  { name: NODE_TYPE_NAMES.HP_MASTERNODE, hint: '4000 HELLAR collateral' },
                ],
          ...
          if (ctx.config.get('core.masternode.enable')) {
            ctx.config.set('platform.drive.tenderhellar.mode', 'validator');
          } else {
            ctx.config.set('platform.drive.tenderhellar.mode', 'full');
          }
  ...
  If you are registering a new HP masternode, hellarmate will provide more
  ...
  transaction.\n
  If you are registering a new masternode, hellarmate will provide more
  information and help you to generate the necessary keys.\n`;
          }
      ...
      },
      {
        task: (ctx, task) => {
          ...
          task.output = chalk`Node configuration completed successfully!
            ...
            You can now run {bold.cyanBright hellarmate start} to start your node, followed by
            {bold.cyanBright hellarmate status} for a node health status overview.

            Run {bold.cyanBright hellarmate --help} or {bold.cyanBright hellarmate <command> --help} for quick help on how
            to use hellarmate to manage your node.\n`;
        },
        options: {
          persistentOutput: true,
          rendererOptions: {
