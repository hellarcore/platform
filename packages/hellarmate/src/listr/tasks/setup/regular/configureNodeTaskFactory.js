...
export default function configureNodeTaskFactory(createIpAndPortsForm) {
  ...
  Your BLS private key can be found in the "hellar.conf" file of your masternode,
  ...
  when initially configuring your masternode.\n`,
          ...
          // Platform Node Key
          if (ctx.isHP) {
            let platformNodeKey = ctx.tenderhellarNodeKey;
            if (!ctx.tenderhellarNodeKey) {
              platformNodeKey = await task.prompt(createPlatformNodeKeyInput({
                initial: ctx.nodeType === NODE_TYPE_MASTERNODE ? '' : undefined,
              }));
            }

            ctx.config.set('platform.drive.tenderhellar.node.id', deriveTenderhellarNodeId(platformNodeKey));
            ctx.config.set('platform.drive.tenderhellar.node.key', platformNodeKey);
          }
          ...
          if (
            ...
            if (ctx.isHP) {
              ctx.config.set('platform.hapi.envoy.http.port', form.platformHTTPPort);
              ctx.config.set('platform.drive.tenderhellar.p2p.port', form.platformP2PPort);
            }
          }
