...
export default async function getConfigurationOutputFromContext(ctx) {
  ...
  if (ctx.nodeType === NODE_TYPE_MASTERNODE) {
    ...
    output += chalk`Operator Public key: ${publicKeyHex}
                    Operator Private key: ******** - view with {bold.cyanBright hellarmate config get core.masternode.operator.privateKey}\n\n`;
  }

  if (ctx.isHP) {
    output += chalk`Platform Node ID: ${ctx.config.get('platform.drive.tenderhellar.node.id')}
                    Platform Node key: ******** - view with {bold.cyanBright hellarmate config get platform.drive.tenderhellar.node.key}\n\n`;
  }
  ...
  if (ctx.isHP) {
    output += `\n\nPlatform P2P port: ${ctx.config.get('platform.drive.tenderhellar.p2p.port')}
                   Platform HTTP port: ${ctx.config.get('platform.hapi.envoy.http.port')}`;
  }
