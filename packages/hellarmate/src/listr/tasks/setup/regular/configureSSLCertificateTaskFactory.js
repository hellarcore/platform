...
export default function configureSSLCertificateTaskFactory(
    ...
    const providerTasks = {
      ...
      [SSL_PROVIDERS.ZEROSSL]: {
        ...
        task: async (ctx, task) => {
          ...
          ctx.config.set('platform.hapi.envoy.ssl.providerConfigs.zerossl.apiKey', apiKey);
    ...
    return new Listr([
      {
        ...
        task: async (ctx, task) => {
          ...
          let header = `  Evonodes are required use TLS encryption on the HAPI
  ...
  (CA). Hellarmate offers three options to configure this certificate:

    ZeroSSL      - Provide a ZeroSSL API key and let hellarmate configure the certificate
                   https://zerossl.com/documentation/api/ ("Access key" section)
    File on disk - Provide your own certificate to hellarmate\n`;
          ...
          ctx.config.set('platform.hapi.envoy.ssl.provider', ctx.certificateProvider);
