...
import BlsSignatures from '@hellarpro/bls';
import {
...
export default function registerMasternodeWithCoreWalletFactory(createIpAndPortsForm) {
  ...
  async function registerMasternodeWithCoreWallet(ctx, task, defaultConfigs) {
    ...
    const collateralDenomination = ctx.preset === PRESET_MAINNET ? 'HELLAR' : 'tHELLAR';
    ...
    let instructionsUrl = 'https://docs.hellar.io/mn-setup-core-collateral';
    if (ctx.isHP) {
      instructionsUrl = 'https://docs.hellar.io/evonode-setup-core-collateral';
    }
      ...
      const prompts = [
        {
          type: 'form',
          name: 'collateral',
          header: `  Hellarmate needs to collect your collateral funding transaction hash and index.
  The funding value must be exactly ${collateralAmount} ${collateralDenomination}.

  Please follow the instructions on how to create a collateral funding transaction in Hellar Core Wallet:
  ${instructionsUrl}\n`,
        ...
        },
        {
          type: 'form',
          name: 'keys',
          header: `  Hellarmate needs to collect details about the owner, voting and payout addresses
  to use in the masternode registration transaction. These are regular Hellar
  addresses, encoded in base58 format.\n`,
        ...
        },
        {
          type: 'form',
          name: 'operator',
          header: `  Hellarmate needs to collect details on the operator key and operator reward share
  to use in the registration transaction. The operator key is a BLS private key,
  encoded in hexadecimal format. Hellarmate will record the private key in the masternode
  ...
  masternode reward to pay to the operator.\n`,
      ...
      const platformP2PPort = state.ipAndPorts.platformP2PPort
        || defaultConfigs.get(ctx.preset)
          .get('platform.drive.tenderhellar.p2p.port');

      const platformHTTPPort = state.ipAndPorts.platformHTTPPort
        || defaultConfigs.get(ctx.preset)
          .get('platform.hapi.envoy.http.port');

      let command;
      if (ctx.isHP) {
        command = `hellar-cli protx register_evo \\
  ...
  ${deriveTenderhellarNodeId(state.platformNodeKey)} \\
  ${platformP2PPort} \\
  ${platformHTTPPort}`;
      } else {
        command = `hellar-cli protx register \\
  ${state.collateral.txId} \\
  ${state.collateral.outputIndex} \\
  ${state.ipAndPorts.ip}:${state.ipAndPorts.coreP2PPort} \\
  ${state.keys.ownerAddress} \\
