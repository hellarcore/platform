...
export default function obtainZeroSSLCertificateTaskFactory(
  ...
  async function obtainZeroSSLCertificateTask(config) {
    ...
    const apiKey = config.get('platform.hapi.envoy.ssl.providerConfigs.zerossl.apiKey', true);
    const externalIp = config.get('externalIp', true);

    const sslConfigDir = homeDir.joinPath(config.getName(), 'platform', 'hapi', 'envoy', 'ssl');
    ...
    return new Listr([
      {
        ...
        skip: (ctx) => ctx.force,
        task: async (ctx, task) => {
          const certificateId = await config.get('platform.hapi.envoy.ssl.providerConfigs.zerossl.id');
          ...
          if (certificate.common_name !== externalIp) {
            ...
            + ' certificate in the ZeroSSL hellarboard');
          }
      ...
      },
      {
        ...
        task: async (ctx) => {
          ...
          config.set('platform.hapi.envoy.ssl.enabled', true);
          config.set('platform.hapi.envoy.ssl.provider', 'zerossl');
          config.set('platform.hapi.envoy.ssl.providerConfigs.zerossl.id', ctx.certificate.id);
        },
      },
      {
    ...
    and all Hellar service ports listed above.`,
                  message: 'Try again?',
                  enabled: 'Yes',
                  disabled: 'No',
                  initial: true,
