...
export default class VerificationServer {
  ...
  async setup(config, validationUrl, validationContent) {
    ...
    // Set up Envoy config
    const configSubPath = path.join('platform', 'hapi', 'envoy');
    ...
    const configDir = this.homeDir.joinPath(config.getName(), 'platform', 'hapi', 'envoy');
  ...
  async start() {
    ...
    const image = this.config.get('platform.hapi.envoy.docker.image');

    const name = 'hellarmate-zerossl-validation';
