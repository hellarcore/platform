...
export default function reindexNodeTaskFactory(
  ...
  /**
   * Gets hellarcore docker container from the node
   * @param config
  ...
  function reindexNodeTask(config) {
    return new Listr([
      ...
      },
      {
        ...
        task: async () => {
          // Temporary (for the first run) set reindex=1 to hellard.conf
          const configPath = 'core/hellar.conf';
      ...
      },
      {
        ...
        task: async (ctx) => {
          ...
          // When Core is started remove reindex=1 from hellard.conf
