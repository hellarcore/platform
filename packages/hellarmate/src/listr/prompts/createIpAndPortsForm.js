...
export default function createIpAndPortsFormFactory(defaultConfigs) {
  ...
  async function createIpAndPortsForm(network, options = {}) {
    ...
    function validatePlatformP2PPort(value) {
      ...
      if (network !== PRESET_MAINNET
        && value === mainnetCfg.get('platform.drive.tenderhellar.p2p.port').toString()) {
    ...
    if (options.isHPMN) {
      ...
      if (initialPlatformP2PPort === null
        ...
        || network === PRESET_MAINNET) {
        initialPlatformP2PPort = defaultConfigs.get(network).get('platform.drive.tenderhellar.p2p.port').toString();
      }
      ...
      if (initialPlatformHTTPPort === null
        ...
        || network === PRESET_MAINNET) {
        initialPlatformHTTPPort = defaultConfigs.get(network).get('platform.hapi.envoy.http.port').toString();
      }
    ...
    return {
      type: 'form',
      name: 'ipAndPorts',
      header: `  Hellarmate needs to collect your external public static IP address and port
