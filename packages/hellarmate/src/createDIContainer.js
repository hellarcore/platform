...
import createTenderhellarRpcClient from './tenderhellar/createTenderhellarRpcClient.js';
...
import configureTenderhellarTaskFactory from './listr/tasks/setup/local/configureTenderhellarTaskFactory.js';
...
export default async function createDIContainer(options = {}) {
  ...
  container.register({
    ...
    homeDir: asFunction(() => (
      HomeDir.createWithPathOrDefault(options.HELLARMATE_HOME_DIR)
    )).singleton(),
    ...
    isHelper: asValue(process.env.HELLARMATE_HELPER === '1'),
   ...
   */
  container.register({
    createTenderhellarRpcClient: asValue(createTenderhellarRpcClient),
  });
  ...
  container.register({
    ...
    configureTenderhellarTask: asFunction(configureTenderhellarTaskFactory).singleton(),
