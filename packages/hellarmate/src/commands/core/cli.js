...
export default class CliCommand extends ConfigBaseCommand {
  static description = 'Hellar Core CLI';

  static args = {
    command: Args.string({
      name: 'command',
      required: true,
      description: 'hellar core command written in the double quotes',
    }),
  };
  ...
  async runWithDependencies(
    ...
    const { out } = await dockerCompose.execCommand(config, 'core', `hellar-cli ${command}`);
