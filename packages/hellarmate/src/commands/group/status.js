    ...
    for (const scopeResult of scopeResults) {
      ...
      if (flags.format === OUTPUT_FORMATS.PLAIN) {
        ...
        if (scope.platform.enabled) {
          if (scope.platform.tenderhellar.serviceStatus === ServiceStatusEnum.error) {
            plain['Platform Container'] = scope.platform.tenderhellar.dockerStatus || 'n/a';
            plain['Platform Status'] = colors.status(scope.platform.tenderhellar.serviceStatus)(scope.platform.tenderhellar.serviceStatus) || 'n/a';
          } else {
            plain['Platform Status'] = colors.status(scope.platform.tenderhellar.serviceStatus)(scope.platform.tenderhellar.serviceStatus) || 'n/a';
            plain['Platform Version'] = scope.platform.tenderhellar.version || 'n/a';
            plain['Platform Block Height'] = scope.platform.tenderhellar.latestBlockHeight || 'n/a';
            plain['Platform Peers'] = scope.platform.tenderhellar.peers || 'n/a';
            plain['Platform Network'] = scope.platform.tenderhellar.network || 'n/a';
          }
        }
