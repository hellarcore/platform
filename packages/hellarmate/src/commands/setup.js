...
export default class SetupCommand extends BaseCommand {
  static description = 'Set up a new Hellar node';
          ...
          task: async (ctx, task) => {
            if (ctx.preset === undefined) {
              ctx.preset = await task.prompt([
                {
                  type: 'select',
                  header: `  Hellarmate provides three default configuration presets:

    mainnet - Run a node connected to the Hellar main network
    testnet - Run a node connected to the Hellar test network
    local   - Run a full network environment on your machine for local development\n`,
            ...
            if (isAlreadyConfigured) {
              const resetCommand = ctx.preset === PRESET_LOCAL
                ? `hellarmate group reset --group ${ctx.preset} --hard` : `hellarmate reset --config ${ctx.preset} --hard`;
  ...
  If you want to keep the existing data and configuration, please use the {bold.cyanBright hellarmate config create}
  command to create a new configuration for this preset.`;
    ...
    if (!isVerbose) { // TODO: We need to print it only with default renderer
      ...
      const welcomeText = begoo(
        chalk`Hello! I'm your {bold.cyanBright Hellar} mate!

I will assist you with setting up a Hellar node on mainnet or testnet. I can also help you set up a development network on your local system.`,
        { maxLength: 45 },
      );
