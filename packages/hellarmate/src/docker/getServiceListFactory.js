...
import { HELLARMATE_HELPER_DOCKER_IMAGE, PACKAGE_ROOT_DIR } from '../constants.js';
  ...
  function getServiceList(config) {
    ...
    return Object
      ...
      .map(([serviceName, { image: serviceImage, labels, profiles: serviceProfiles }]) => {
        const title = labels?.['org.hellarmate.service.title'];

        if (!title) {
          throw new Error(`Label for hellarmate service ${serviceName} is not defined`);
        }

        // Use hardcoded version for hellarmate helper
        // Or parse image env variable name and extract version from the env
        const image = serviceName === 'hellarmate_helper'
          ? HELLARMATE_HELPER_DOCKER_IMAGE : envs[serviceImage.match(/([A-Z_]+)/)[0]];
