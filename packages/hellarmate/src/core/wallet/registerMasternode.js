...
export default async function registerMasternode(
  ...
  const proTxArgs = [
    ...
    collateralHash, // The txid of the 1000 Hellar collateral funding transaction
    parseInt(collateralOutputIndex, 10), // The output index of the 1000 Hellar funding transaction
    ipAndPort, // Masternode IP address and port, in the format x.x.x.x:yyyy
    ownerAddress, // The new Hellar address for the owner/voting address
    operatorPublicKey, // The Operator BLS public key
    ownerAddress, // The new Hellar address, or the address of a delegate, used for proposal voting
    operatorReward, // The percentage of the block reward allocated to the operator as payment
    fundSourceAddress, // A new or existing Hellar address to receive the owner’s masternode rewards
  ];

  if (hp) {
    const platformNodeId = config.get('platform.drive.tenderhellar.node.id');
    const platformP2PPort = config.get('platform.drive.tenderhellar.p2p.port');
    const platformHttpPort = config.get('platform.hapi.envoy.http.port');
