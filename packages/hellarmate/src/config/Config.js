...
const {
  get: lohellarGet, set: lohellarSet, cloneDeep: lohellarCloneDeep, isEqual: lohellarIsEqual,
} = lohellar;
...
export default class Config {
   ...
   */
  has(path) {
    return lohellarGet(this.options, path) !== undefined;
  }
  ...
  get(path, isRequired = false) {
    const value = lohellarGet(this.options, path);
   ...
   */
  set(path, value) {
    const clonedOptions = lohellarCloneDeep(this.options);

    lohellarSet(clonedOptions, path, lohellarCloneDeep(value));
   ...
   */
  setOptions(options) {
    const clonedOptions = lohellarCloneDeep(options);
   ...
   */
  isEqual(config) {
    return lohellarIsEqual(this.getOptions(), config.getOptions());
  }
