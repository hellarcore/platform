...
export default function scheduleRenewZeroSslCertificateFactory(
  ...
  async function scheduleRenewZeroSslCertificate(config) {
    const certificate = await getCertificate(
      config.get('platform.hapi.envoy.ssl.providerConfigs.zerossl.apiKey', false),
      config.get('platform.hapi.envoy.ssl.providerConfigs.zerossl.id', false),
    );
    ...
    const job = new CronJob(expiresAt, async () => {
      ...
      await dockerCompose.execCommand(config, 'hapi_envoy', 'kill -SIGHUP 1');
